...
explicit flag comes from ``GALAXY_API_KEY``, ``GALAXY_API_URL`` or
``GALAXY_READ_ONLY``, and failing those from the built-in defaults
(server ``us1``, read-only off, no API key).
"""
