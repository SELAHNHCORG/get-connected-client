...
:class:`~get_connected_client.resources.users.Users` deliberately leaves out.
"""
...
class Auth(Resource[LoginResult]):
    ...
    with :class:`~get_connected_client.resources.users.Users`'s 21 of 23, the
    ...
    def login(
        ...
        Prefer :meth:`~get_connected_client.client.GalaxyClient.login`: it does
        the same exchange and then *adopts* the token, so subsequent
        requests authenticate. This method only returns it.

        :return: a :class:`~get_connected_client.models.auth.LoginResult` when
    ...
    def authenticate(self, user_email: str, user_password: str) -> list[UserOneclick]:
        ...
        :class:`~get_connected_client.models.users.UserOneclick` rows rather
