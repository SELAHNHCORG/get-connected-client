...
``GALAXY_API_KEY=test-key`` and clears ``GALAXY_API_URL`` for every test in
the suite -- exactly right for the respx-mocked unit tests, exactly wrong
here: it would stomp the user's real ``GALAXY_API_KEY`` with a fake one
before a live test ever ran.
...
def _isolate_env():
    ...
    GALAXY_API_KEY or clear GALAXY_API_URL like the parent fixture does."""
    yield
